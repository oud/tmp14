...
class EmailResourceIT {
    ...
    void getAllEmails() throws Exception {
        ...
        restEmailMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(email.getId().toString())))
    ...
    void getNonExistingEmail() throws Exception {
        // Get the email
        restEmailMockMvc.perform(get(ENTITY_API_URL_ID, UUID.randomUUID().toString())).andExpect(status().isNotFound());
    }
    ...
    void putWithIdMismatchEmail() throws Exception {
        ...
        restEmailMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(emailDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteEmail() throws Exception {
        ...
        // Delete the email
        restEmailMockMvc
            .perform(delete(ENTITY_API_URL_ID, email.getId().toString()).accept(MediaType.APPLICATION_JSON))
