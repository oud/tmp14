...
class AdresseResourceIT {
    ...
    void getAllAdresses() throws Exception {
        ...
        restAdresseMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(adresse.getId().toString())))
    ...
    void getAdresse() throws Exception {
        ...
        restAdresseMockMvc
            ...
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(adresse.getId().toString()))
    ...
    void getNonExistingAdresse() throws Exception {
        // Get the adresse
        restAdresseMockMvc.perform(get(ENTITY_API_URL_ID, UUID.randomUUID().toString())).andExpect(status().isNotFound());
    }
    ...
    void putWithIdMismatchAdresse() throws Exception {
        ...
        restAdresseMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(adresseDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteAdresse() throws Exception {
        ...
        // Delete the adresse
        restAdresseMockMvc
            .perform(delete(ENTITY_API_URL_ID, adresse.getId().toString()).accept(MediaType.APPLICATION_JSON))
