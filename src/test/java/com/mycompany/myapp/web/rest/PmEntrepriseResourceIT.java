...
class PmEntrepriseResourceIT {
    ...
    void getAllPmEntreprises() throws Exception {
        ...
        restPmEntrepriseMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(pmEntreprise.getId().toString())))
    ...
    void getPmEntreprise() throws Exception {
        ...
        restPmEntrepriseMockMvc
            ...
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(pmEntreprise.getId().toString()))
    ...
    void getNonExistingPmEntreprise() throws Exception {
        // Get the pmEntreprise
        restPmEntrepriseMockMvc.perform(get(ENTITY_API_URL_ID, UUID.randomUUID().toString())).andExpect(status().isNotFound());
    }
    ...
    void putNonExistingPmEntreprise() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        pmEntreprise.setId(UUID.randomUUID());
    ...
    void deletePmEntreprise() throws Exception {
        ...
        restPmEntrepriseMockMvc
            .perform(delete(ENTITY_API_URL_ID, pmEntreprise.getId().toString()).accept(MediaType.APPLICATION_JSON))
